...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="relative w-full md:w-auto">
            ...
            <input type="text" id="searchKendaraan" placeholder="Cari kendaraan..." class="w-full sm:w-72 rounded-lg border border-gray-300 py-2 pl-10 pr-4 focus:border-[#5AA8D6] focus:outline-none">
        </div>
    </div>
        ...
        <table class="min-w-full text-sm whitespace-nowrap">
            ...
            <tbody id="kendaraanTable">
                ...
                <tr class="kendaraan-row hover:bg-gray-50">
                    ...
                    <td class="border px-4 py-3 text-center font-semibold text-gray-700">{{ $item->plat_nomor }}</td>
                    <td class="border px-4 py-3 text-gray-600">{{ $item->merk }}</td>
                    <td class="border px-4 py-3">
                        <div class="flex justify-center gap-2">
                            <a href="{{ route('admin.kendaraan.edit', $item->id) }}" class="rounded-lg bg-yellow-400 px-3 py-2 text-white transition hover:bg-yellow-500">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            <form action="{{ route('admin.kendaraan.destroy', $item->id) }}" method="POST" class="form-delete inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="rounded-lg bg-red-500 px-3 py-2 text-white transition hover:bg-red-600">
                ...
                @empty
                <tr>
                    <td colspan="6" class="border px-4 py-8 text-center text-gray-500">Data kendaraan belum tersedia.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
